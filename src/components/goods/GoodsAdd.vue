<template>
    ...
    <Row>
        ...
        <Col span="16">
          <Select v-model="formItem.categoryId" style="width:200px">
              <div v-for="item in categorys">
                <Option :value="item.id" :key="item.id" disabled>{{ item.name }}</Option>
                <Option v-for="(v,i) in item.childCategories" :value="v.id" :key="v.id">&nbsp;&nbsp;&nbsp;{{ v.name }}</Option>
              </div>
          </Select>
        </Col>
      </Row>
      ...
      <Row class="margin-10">
          <Col span="4">
            <p class="input-text">外部编号</p>
          </Col>
          <Col span="10">
            <Input v-model="formItem.number" style="width: 300px"></Input>
          </Col>
        </Row>
...
export default{
  ...
  mounted(){
    ...
    qiniu(){
      ...
      editor.customConfig.uploadImgHooks = {
          ...
          customInsert: function (insertImg, result, editor) {
              ...
              var url = 'http://ovweugbfd.bkt.clouddn.com/'+result.key
              insertImg(url)
    ...
    handleSuccess (res, file) {
      console.log(res.key);
      console.log(file);
        file.url = 'http://ovweugbfd.bkt.clouddn.com/'+res.key;
    ...
    save(){
      ...
      params.append('number', this.formItem.number);
