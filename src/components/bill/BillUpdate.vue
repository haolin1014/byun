          ...
          <Col span="10">
              <table class="table">
                <tr v-for="(v,i) in formItem.carts">
                  ...
                  <td><Icon type="minus-circled" size="15" @click.native="minus(v)"></Icon> <input type="number" width="30" v-model="v.quantity"> <Icon type="plus-circled" size="15" @click.native="plus(v)" ></Icon></td>
                  <td>￥{{v.amount}}</td>
                  <td>
  ...
  methods:{
    dataInit(){
      ...
      axios.get(URL+'list/'+id)
        ...
        console.log(this.formItem.intervalInt);
    ...
    saveInterval(){
      ...
      var id = this.$route.params.id;
      var interval = this.formItem.intervalInt;
